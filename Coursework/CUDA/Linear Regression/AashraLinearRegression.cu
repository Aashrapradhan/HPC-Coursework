...
int num_data = 1000;
__device__ int d_num_data =1000;
...
__device__ double d_residual_error(double P, double Q, double m, double c) {
  double e = (m * P) + c - Q;
  return e * e;
}
double rms_error(double m, double c) {
  ...
  for(aa=0; aa<num_data; aa++) {
    error_sum += residual_error(data[aa].P, data[aa].Q, m, c);  
  }
...
__global__ void d_rms_error(double *m, double *c,double *error_sum_arr,point_t *d_data) {
  int aa = threadIdx.x + blockIdx.x *blockDim.x;
error_sum_arr[aa] = d_residual_error(d_data[aa].P,d_data[aa].Q, *m, *c);
}
...
d_rms_error <<<100,10>>>(&d_dm[aa],&d_dc[aa],d_error_sum_arr,d_data);
...
error_sum_mean = error_sum_total / num_data;
e[aa] =sqrt(error_sum_mean);

if(e[aa] < best_error){
best_error = e[aa];
error_sum_total +=h_error_sum_arr[aa];
}
error_sum_mean = error_sum_total /num_data;
e[aa] =  sqrt(error_sum_mean); 
