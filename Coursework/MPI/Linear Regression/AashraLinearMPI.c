...
* To compile:
*   mpicc -o AashraLinearMPI AashraLinearMPI.c -lm
* 
* To run:
*   mpirun -n 9 ./AashraLinearMPI
...
double residual_error (double p, double q, double m, double c)
{
   double e = (m * p) + c - q;
   return e * e;
}
...
double rms_error (double m, double c)
{
   ...
   for (aashra = 0; aashra < n_data; aashra++)
   {
      error_sum += residual_error (data[aashra].p, data[aashra].q, m, c);
   }
...
int main () {
   ...
   while (!minimum_found)
   {
      ...
      if (rank != 0)
      {
         ...
         pError = rms_error (dm[aashra], dc[aashra]);

         MPI_Send (&pError, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
         MPI_Send (&dm[aashra], 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
         MPI_Send (&dc[aashra], 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
      ...
      {
         for (aashra = 1; aashra < size; aashra++)
         {
            MPI_Recv (&pError, 1, MPI_DOUBLE, aashra, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv (&dm[aashra-1], 1, MPI_DOUBLE, aashra, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv (&dc[aashra-1], 1, MPI_DOUBLE, aashra, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            if (pError < best_error)
            {
               best_error = pError;
               best_error_aashra = aashra - 1;
         ...
         for (aashra = 1; aashra < size; aashra++)
         {
            MPI_Send (&bm, 1, MPI_DOUBLE, aashra, 0, MPI_COMM_WORLD);
            MPI_Send (&bc, 1, MPI_DOUBLE, aashra, 0, MPI_COMM_WORLD);
            MPI_Send (&minimum_found, 1, MPI_INT, aashra, 0, MPI_COMM_WORLD);
         }
      }
   }
